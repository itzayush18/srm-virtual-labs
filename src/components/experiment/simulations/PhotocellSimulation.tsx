...
const PhotocellSimulation = () => {
  ...
  type ChartDataPoint = { wavelength: number; intensity: number; current: number };
  ...
  }, [wavelength, intensity, material, voltage, materials]);
  ...
  const addDataPoint = () => {
    ...
    setChartData(prevData => {
      ...
      const existingIndex = prevData.findIndex(
        p => Math.abs(p.wavelength - wavelength) < 5 && Math.abs(p.intensity - intensity) < 1
      );
  ...
  const filteredSpectralData = chartData
    .filter(p => Math.abs(p.intensity - intensity) < 1)
      ...
      <div className="space-y-6">
        <div className="data-panel">
          ...
          <div className="h-80 bg-gray-100 border rounded-md">
            ...
            >
              <PhotocellScene wavelength={wavelength} intensity={intensity} current={current} />
            </Suspense>
          </div>
        </div>
        ...
        <div className="data-panel">
            ...
            {filteredSpectralData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={filteredSpectralData}>
                  ...
                  />
                  <YAxis label={{ value: 'Current (μA)', angle: -90, position: 'insideLeft' }} />
                  <Tooltip
                    formatter={value => {
          ...
          <div className="mt-4 p-3 bg-gray-50 rounded-md">
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              <div>
                Peak Sensitivity: {materials[material as keyof typeof materials].peakWavelength} nm
              </div>
              <div>Bandgap: {materials[material as keyof typeof materials].bandgap} eV</div>
              <div>
                Responsivity: {materials[material as keyof typeof materials].responsivity} A/W
              </div>
              <div>
                Cutoff Wavelength: {materials[material as keyof typeof materials].cutoffWavelength}{' '}
                nm
              </div>
            </div>
          </div>
