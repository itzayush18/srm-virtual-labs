...
const scatteringMechanisms = [
  { id: 'phonon', name: 'Phonon Scattering', description: 'Scattering by lattice vibrations', temperatureDependence: -1.5 },
  { id: 'impurity', name: 'Impurity Scattering', description: 'Scattering by dopant ions', temperatureDependence: 1.5 },
  { id: 'carrier', name: 'Carrier-Carrier Scattering', description: 'Interactions between carriers', temperatureDependence: 0 }
];
...
const CarrierMobilitySimulation = () => {
  ...
  const calculateCarrierMobility = (temp: number, doping: number, material: string) => {
  ...
  }, [temperature, dopingConcentration, materialType, activeScatteringMechanisms]);
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center space-x-2">
              ...
              />
              <span className="min-w-[80px] text-right">{formatDopingConcentration(dopingConcentration)}</span>
            </div>
          </div>
              ...
              {scatteringMechanisms.map(mechanism => (
                <div key={mechanism.id} className="flex items-center">
                  <input 
                    type="checkbox" 
                    id={mechanism.id} 
                    checked={activeScatteringMechanisms[mechanism.id as keyof typeof activeScatteringMechanisms]}
        ...
        <Card className="p-4">
          ...
          <div className="grid grid-cols-2 gap-2 text-sm">
            <div>Bandgap:</div>
            <div>{materialProperties[materialType as keyof typeof materialProperties].bandgap} eV</div>
            
            <div>Effective Mass:</div>
            <div>{materialProperties[materialType as keyof typeof materialProperties].effectiveMass} m₀</div>
            
            <div>Max Mobility:</div>
            <div>{materialProperties[materialType as keyof typeof materialProperties].baseCarrierMobility} cm²/Vs</div>
          </div>
        </Card>
      </div>
        ...
        <div className="bg-gray-50 p-4 rounded-md">
          <div className="mb-4">
            ...
            <div className="text-3xl font-bold text-lab-teal">
              {Math.round(calculateCarrierMobility(temperature, dopingConcentration, materialType))} cm²/Vs
            </div>
          </div>
          
          <div className="text-sm space-y-1">
            <div>Material: {materialProperties[materialType as keyof typeof materialProperties].name}</div>
          ...
          {chartData.length > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData}>
                ...
                />
                <YAxis 
                  label={{ value: 'Mobility (cm²/Vs)', angle: -90, position: 'insideLeft' }} 
                />
