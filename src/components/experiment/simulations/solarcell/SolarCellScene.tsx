...
const SolarPanel = ({ 
  ...
  const powerOutput = maxPower * efficiency * lightIntensity * 
                      (1 - Math.max(0, (temperature - 25) * 0.005));
...
const LoadGraph = ({ 
  ...
  for (let v = 0; v <= maxVoltage; v += maxVoltage / 20) {
    const i = v <= voltage ? Math.sqrt(power / resistance) : Math.sqrt(power / resistance) * (1 - (v - voltage) / voltage);
  ...
  return (
    <group position={position}>
      ...
      {/* Max power point */}
      <mesh position={[voltage / maxVoltage * 3 - 1.5, current / current * 0.8 - 1 + 0.8, 0.01]} >
