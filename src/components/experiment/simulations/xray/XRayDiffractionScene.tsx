      ...
      for (let j = 0; j < size; j++) {
        for (let k = 0; k < size; k++) {
          ...
          if (structureType === 'fcc' || structureType === 'diamond') {
            positions.push([x + latticeConstant / 2, y + latticeConstant / 2, z]);
            positions.push([x + latticeConstant / 2, y, z + latticeConstant / 2]);
            positions.push([x, y + latticeConstant / 2, z + latticeConstant / 2]);
          }
    ...
    <group ref={latticeRef} position={position}>
      ...
      {atomPositions.map(([x, y, z], index) => (
        <mesh key={`atom-${index}`} position={[x, y, z]}>
          ...
          <meshPhysicalMaterial color={atomColor} metalness={0.3} roughness={0.2} clearcoat={0.5} />
        </mesh>
      ))}
      ...
      {bonds.map(({ start, end }, index) => {
        ...
        return (
          <mesh key={`bond-${index}`} position={[midX, midY, midZ]} quaternion={quaternion}>
            <cylinderGeometry args={[bondRadius, bondRadius, length, 8]} />
            <meshStandardMaterial color="#aaaaaa" metalness={0.2} roughness={0.8} />
          </mesh>
        );
      })}
...
const DiffractionPattern = ({
  ...
  return (
    <group ref={patternRef} position={position}>
      ...
      {/* Title */}
      <Text position={[0, 1.7, 0]} fontSize={0.2} color="#fff" anchorX="center">
        X-Ray Diffraction Pattern
      </Text>

      {/* X-axis title */}
      <Text position={[0, -1.9, 0]} fontSize={0.15} color="#fff" anchorX="center">
        2θ (degrees)
      </Text>
...
const XRaySetup = ({
  ...
  // X-ray beam path
  const beamPath = new THREE.QuadraticBezierCurve3(sourcePos, samplePos, detectorPos);
  ...
  return (
    <group position={position}>
      ...
      {/* Labels */}
      <Text position={[-2, -0.6, 0]} fontSize={0.15} color="#fff" anchorX="center">
        X-Ray Source
      </Text>

      <Text position={[0, -0.6, 0]} fontSize={0.15} color="#fff" anchorX="center">
        Sample
      </Text>
      ...
      <Text position={[0, 0.8, 0]} fontSize={0.15} color="#fff" anchorX="center">
...
const XRayDiffractionScene: React.FC<{
  ...
  return (
    <Canvas shadows className="h-80 w-full">
      ...
      <OrbitControls enablePan enableZoom enableRotate minPolarAngle={0} maxPolarAngle={Math.PI} />
      ...
      <XRaySetup wavelength={wavelength} position={[0, -2.5, 0]} />
