...
const DiodeModel = ({ 
  ...
  return (
    <group ref={diodeRef} position={position}>
      ...
      {isForwardBiased && Array.from({ length: 5 }).map((_, i) => (
        <mesh 
          key={i}
          position={[
            -2 + ((clockRef.current.getElapsedTime() * normalizedCurrent * 2 + i * 0.7) % 4) - 0.5,
            (i % 2 === 0 ? 0.2 : -0.2),
            (i % 3 === 0 ? 0.2 : -0.2)
          ]}
        >
...
const IVCurve = ({
  ...
  const logSelectedCurrent = Math.sign(selectedCurrent) * Math.log10(1 + Math.abs(selectedCurrent) * 1e9) / 9;
