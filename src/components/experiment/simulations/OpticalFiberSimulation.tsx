...
const OpticalFiberSimulation = () => {
  ...
  const addDataPoint = () => {
    ...
    setChartData(prevData => {
      const existingIndex = prevData.findIndex(p => 
        ...
        Math.abs(p.length - fiberLength) < 0.1
      );
  ...
  const exportData = () => {
    const csvData = [
      ['Wavelength (nm)', 'Input Power (mW)', 'Output Power (mW)', 'Attenuation (dB/km)', 'Fiber Length (km)'],
      ...chartData.map(point => [
        point.wavelength, 
        point.inputPower, 
        point.outputPower, 
      ...
      <div className="control-panel">
        ...
        <div className="space-y-6">
          <div className="space-y-2">
            ...
            <div className="h-2 w-full rounded-full" style={{ background: `linear-gradient(to right, violet, blue, cyan, green, yellow, orange, red, rgba(0,0,0,0.2), rgba(0,0,0,0.2))` }}></div>
          </div>
